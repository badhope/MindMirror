...
export default function DashboardPage() {
  ...
  const { completedAssessments, deleteAssessment, records } = useAssessmentStore((state) => state)
  const { achievements } = useAchievementStore((state) => state)
  
  const [showProfile, setShowProfile] = useState(false)

  const stats = useAssessmentStore((state) => {
            ...
            <div className="flex flex-col lg:flex-row gap-8">
              ...
              <div className="flex-1 grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                  ...
                  { value: 0, label: '收藏', color: 'text-pink-400' },
