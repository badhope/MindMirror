...
export async function exportToJSON(
  ...
  return {
    success: true,
    data: blob,
    filename: `mindmirror-export-${new Date().toISOString().split('T')[0]}.json`,
...
export async function exportToCSV(
  ...
  return {
    success: true,
    data: blob,
    filename: `mindmirror-export-${new Date().toISOString().split('T')[0]}.csv`,
    mimeType: 'text/csv',
  }
}
...
<head>
  ...
  <title>${opts.language === 'zh' ? '心镜 MindMirror 测评报告' : '心镜 MindMirror Assessment Report'}</title>
  <style>
...
<body>
  <div class="container">
    ...
    <div class="footer">
      <p>${opts.language === 'zh' ? '由 心镜 MindMirror 生成 | 探索你的无限可能' : 'Generated by 心镜 MindMirror | Explore Your Infinite Possibilities'}</p>
  ...
  return {
    success: true,
    data: htmlTemplate,
    filename: `mindmirror-report-${new Date().toISOString().split('T')[0]}.html`,
    mimeType: 'text/html',
  }
}
...
export async function exportToMarkdown(
  ...
  let mdContent = `# 🧠 心镜 MindMirror ${opts.language === 'zh' ? '测评报告' : 'Assessment Report'}\n\n`
  ...
  mdContent += `\n---\n\n*${opts.language === 'zh' ? '由 心镜 MindMirror 生成' : 'Generated by MindMirror'}* | ${new Date().toLocaleString(opts.language === 'zh' ? 'zh-CN' : 'en-US')}\n`
  ...
  return {
    success: true,
    data: blob,
    filename: `mindmirror-report-${new Date().toISOString().split('T')[0]}.md`,
      ...
      canvas.toBlob((blob) => {
        if (blob) {
          resolve({
            success: true,
            data: blob,
            filename: `mindmirror-report-${new Date().toISOString().split('T')[0]}.png`,
            mimeType: 'image/png',
          })
        } else {
          resolve({
