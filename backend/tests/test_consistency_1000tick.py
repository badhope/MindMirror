...
# =============================================================================
#  1000 Tick 双写一致性验证
#  前端 TypeScript <-> 后端 Python 数值一致性验证
#  允许误差: < 0.01%
# =============================================================================
"""
运行方法:
cd backend
python -m pytest tests/test_consistency_1000tick.py -v --tb=short
"""
...
def load_test_fixture():
    fixture_path = Path(__file__).parent / "fixtures" / "initial_state.json"
    if fixture_path.exists():
        with open(fixture_path, "r", encoding="utf-8") as f:
            return json.load(f)
    
    return generate_minimal_test_state()
...
def assert_near_equal(a: float, b: float, tolerance: float = 0.01, msg: str = ""):
    if b == 0:
        rel_error = abs(a - b)
    else:
        rel_error = abs(a - b) / abs(b) * 100
    
    if rel_error > tolerance and abs(a - b) > 0.01:
        raise AssertionError(
            f"{msg}\n"
            f"  数值不一致: {a:.6f} vs {b:.6f}\n"
            f"  相对误差: {rel_error:.4f}% (允许: {tolerance}%)"
        )


class TestSingleTickConsistency:
    def test_simple_day_advance(self):
        state = generate_minimal_test_state()
        initial_gold = state["treasury"]["gold"]
        initial_tick = state["tick"]
        
        for _ in range(24):
            state, _ = execute_single_tick_full(state)
        
        assert state["tick"] == initial_tick + 24
        assert state["day"] == 1
        assert state["date"]["day"] == 2
    
    def test_treasury_calculation(self):
        state = generate_minimal_test_state()
        
        for _ in range(24):
            state, _ = execute_single_tick_full(state)
        
        assert state["treasury"]["income"] >= 0
        assert state["treasury"]["expenses"] >= 0
        print(f"\n国库收支:")
        print(f"  收入: {state['treasury']['income']:.2f}")
        print(f"  支出: {state['treasury']['expenses']:.2f}")
        print(f"  余额: {state['treasury']['balance']:.2f}")
        print(f"  黄金: {state['treasury']['gold']:.2f}")


class Test1000TickConsistency:
    def test_1000_tick_stability(self):
        state = generate_minimal_test_state()
        
        print(f"\n{'='*60}")
        print(f"1000 Tick 稳定性测试")
        print(f"{'='*60}")
        ...
        for tick in range(1, 1001):
            ...
            if tick % 100 == 0:
                print(f"\nTick {tick}:")
                print(f"  GDP: {state['stats']['gdp']:.2f}")
                print(f"  黄金: {state['treasury']['gold']:.2f}")
                print(f"  稳定度: {state['stats']['stability']:.1f}")
                print(f"  通胀: {state['stats']['inflation']:.2f}%")
                print(f"  失业: {state['stats']['unemployment']:.2f}%")
        
        print(f"\n{'='*60}")
        print(f"测试完成: {1000} ticks")
        print(f"  总计算时间: {total_time:.2f}ms")
    ...
    def test_determinism(self):
        ...
        assert_near_equal(
            state1["treasury"]["gold"],
            state2["treasury"]["gold"],
            0.001,
            "黄金 确定性计算失败"
        )
...
class TestPerformanceBenchmark:
    def test_performance_1000_ticks(self):
        state = generate_minimal_test_state()
        ...
        print(f"\n性能基准测试:")
        print(f"  1000 ticks 总耗时: {elapsed:.2f}ms")
        print(f"  平均每 tick: {elapsed/1000:.3f}ms")
        print(f"  每秒可计算: {1000000/elapsed:.1f} ticks")
        
        assert elapsed < 5000, f"性能不达标: {elapsed:.2f}ms > 5000ms"
        print(f"\n✅ 性能测试通过!")
